...
#     process = run_subprocess_do_not_wait(command, working_directory)
#     wait_for_termination_output(process, stdout_filename, stderr_filename, timeout)
...
def launch_experiment(config_json_filename):
    ...
    for experiment_name in json_data['experiments']:
        # Let's load what is needed to run the experiment
        ...
        commands_file = open(os.getcwd() + '/commands.log', 'w')
        ...
        commands_file.close()
        print('Starting experiment {}/{} ({} %): {}'.format(curr_experiment,
                                                            nb_experiments,
                                                            (curr_experiment * 100) / nb_experiments,
                                                            experiment_name))
        ...
        if wait_for_batsim_to_open_connection(timeout=socket_creation_timeout):
            ...
            try:
                ...
                if (sched_process.returncode != 0) or (batsim_process.returncode != 0):
                    print('FAILED: bad returncode (batsim={}, sched={})'.format(batsim_process.returncode,
            ...
            print('FAILED: cannot reach batsim socket creation :(')
        batsim_process.kill()
